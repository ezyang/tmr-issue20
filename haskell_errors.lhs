...
\end{Verbatim}
You end up with an exception and there's not much you can do about it.\bay{Well, you can do some things about it} However, it is possible to create a~safe version of \verb+head+ function which will work for all possible inputs without throwing an exception. Such functions are called \emph{total functions}.
...
Failed, modules loaded: none.
\end{Verbatim}
Now that the type annotation is correct, it turns out that there's an error in third line. \emph{Not in scope} means that some variable, function or, as implied in this case, a~data constructor, is not known to the compiler. You certainly know this kind of error from different programming languages. Let's look at the definition of \verb+Maybe+ data type to understand what is wrong:\bay{How do I look up the definition of Maybe?}
...
\end{Verbatim}
Our function \verb+safeHead+ should now work as intended. \bay{Better separation here.} Just remember that there already is a library called \verb+safe+~\cite{he:safe}, which provides different safe versions of originally unsafe functions, so there's no need to actually implement your own.

\bay{This could be a section break.}

Let's recall the not-in-scope error that we saw earlier. It was caused by using a function which didn't exist.\bay{Or, more precisely, it existed as a type constructor, but not a data constructor.} Another common situation in which this error arises is when you use an existing function, but fail to import it. Let's look at a~simple piece of code in which a~different name is given to an already existing function:
...
Failed, modules loaded: none.
\end{Verbatim}
GHCi doesn't know \verb+sort+ function, but it knows \verb+sqrt+ from standard \verb+Prelude+ and suggests that we might have made a typo. The \verb+sort+ function we wanted to use is part of the \verb+Data.List+ package.\bay{How would I find that out? see below...} Since this is not in the standard \verb+Prelude+ it must be explicitly imported:
...
\end{Verbatim}
So if you get a not-in-scope error, but you know that the function really exists, the missing import is most likely the culprit. In case you don't know the name of the package in which a~certain function is located you can use hoogle~\cite{he:hoogle}.\bay{Mention this earlier!}
...
I'll begin exploring errors related to type system with a~very simple example shown in Listing \ref{l:type_class_error1}. It demonstrates what can happen when you pass parameters of the wrong type to a function. Let's break this scary error message into its components. The \verb+<interactive>:23:9:+ says that the error occurred in the interactive mode (GHCi) on the 9th column in the 23rd command entered in the prompt since it was started.\bay{Hey we talked about this in parse errors...} The next two lines (\verb+No+ \verb+instance+ \verb+for+ \verb+(Num+ \verb+Bool)+ \verb+arising+ \verb+from+ \verb+the+ \verb+literal+ \verb+`1'+) are the key to understanding the error message. Even without knowing what it means, it seems clear that there's a~problem with \verb+1+ literal.
...
\end{listing}
which means that in order for literal \verb+1+ to be a valid parameter to \verb+(&&)+, the type returned by \verb+fromInteger+ should be \verb+Bool+. There is one problem though. The \verb+Bool+ type is not an instance of \verb+Num+ type class, as shown in Listing~\ref{l:bool}. It should be \bay{wording} however, since the \verb+fromInteger+ function imposes a~constraint that its return value belongs to \verb+Num+ type class. This is exactly what the error message said! The parameter \verb+1+ that we passed to \verb+(&&)+ should be a~\verb+Bool+ \bay{repetitive}, because \verb+(&&)+ requires it, but \verb+Bool+ is not an instance of \verb+Num+ and so a type error occurs. \bay{Long sentence} The next line of the error message suggests a~solution: \verb+Possible+ \verb+fix:+ \verb+add+ \verb+an+ \verb+instance+ \verb+declaration+ \verb+for+ \verb+(Num+ \verb+Bool)+. Indeed, if we made \verb+Bool+ type an instance of \verb+Num+ type class the problem would be gone. In some cases, this may be the solution. Deriving \verb+Bool+ to be an instance of \verb+Num+ is certainly a good exercise \bay{Well, it's not clear if its a good exercise.} that you can try out. In many other cases however this error means you wanted something different than you actually wrote. Let's assume here that we wanted \verb+1+~to denote logical truth. Fixing that is easy:

\begin{Verbatim}
ghci> True && True
True
\end{Verbatim}
The three remaining lines of the error message provide information about expression that caused the error: \bay{Signpost here. We've solved the problem, but we're going back to talk about the error message. It might be better to jump around before you tackle the heart of the issue.  But then again, you have a nested example, so this is debatable.}
...
\end{listing}
\noindent
The error message is shown in Listing~\ref{l:rigid_types}. There are a~few important things in this message. \bay{that sentence has no content} The \verb+(b ~ a)+ notation indicates equality of the types \verb+a+ and \verb+b+. The compiler uses this notation to say \verb+Could not deduce (b ~ a)+, which means that \verb+a+ and \verb+b+ should be identical, but the type definition we provided does not guarantee it. The requirement of types \verb+a+ and \verb+b+ being of the same type is a~direct consequence of a~type signature of \verb+(==)+ which, as we recall, requires its parameters to be of the same type. The term \verb+rigid type variable+ indicates that our types \verb+a+ and \verb+b+ have been directly specified by the type annotation~\cite{he:pj} and the compiler is not free to unify them with another type. \bay{This might use a footnote. It's pretty advanced. What does unification mean?} We have already seen a correct version of this code, but we can also make it work in a~different way:
...
getFirstLetter = head
\end{Verbatim}
The \verb+head+ function has type \verb+[a] -> a+, which means it takes a list of some type \verb+a+~and returns a~single element of type \verb+a+, not a~list. However, the type signature for \verb+getFirstLetter+ requires that the return argument be a list of \verb+Char+s. Therefore, the provided signature is inconsistent with the actual return type of the function. Haskell will notice that when you try to load the code into GHCi and report an error as shown in Listing~\ref{l:mismatch_error}. GHCi says that the annotation provided by the user means that user expected the type \verb+[Char]+, but the actual inferred type is \verb+Char+. \bay{Rewrite the prior sentence.} There are two possible fixes for this problem. First, if we are really fine with getting a~single \verb+Char+ instead of a~\verb+String+, we can change the type signature to \verb+String -> Char+ (or \verb+[Char] -> Char+, since it's the same \bay{due to type synonyms?}). On the other hand, if we really expect to get a~\verb+String+ from our function, we need to change the implementation. In this case the result of \verb+head+ should be wrapped in a~list, which can be done like this:
\begin{Verbatim}
getFirstLetter :: String -> String
getFirstLetter xs = [head xs]
\end{Verbatim}
or using a~point-free style: \bay{it's the monkey operator!}
...
\end{Verbatim}
As soon as compiler realizes that the first parameter is not a~list, it complains. It doesn't even reach the second parameter to analyze it and infer that the first list should contain \verb+Char+s. That is the reason why it uses a~generic list \verb+[a0]+. \bay{How do I tell that a type is 'generic'?} Listing~\ref{l:wrong_param2} shows the result of swapping the two parameters. This time, the compiler is able to infer the exact type of the elements required in the second list \bay{run-on here} because by the time it reaches the second parameter it has already analyzed the first one and it knows that it was a~list of \verb+Char+s.
...
Another common error related to types is type ambiguity. I'll demonstrate it using the \verb+read+ function, which is used to convert \verb+String+ representation of some value to that value. The \verb+read+ function is defined in the \verb+Read+ type class and has type signature \verb+read :: Read a => String -> a+. All data types within standard Prelude, except function types and \verb+IO+ types, are instances of \verb+Read+. Every type that is an instance of \verb+Read+ type class provides its own definition of \verb+read+ function. \bay{Well, you'll get ambiguous type error even if there was only one instance of Read.} This means that the compiler must know what resulting type to expect in order to call a~correct implementation of the \verb+read+ function. This is polymorphism, which was already discussed when we talked about \verb+fromIntegral+ function. Listing~\ref{l:ambiguity} shows that the error that occurs when the polymorphic type variable cannot be inferred to a~concrete type. In this case, the compiler doesn't know which version of \verb+read+ function should be called and \bay{long sentence} suggests to solve the problem by adding a~type signature. Let's try that:
...
safeHead
\end{Verbatim}
The argument \verb+[1,2,3]+ passed to the function \verb+safeHead+ couldn't be matched against any pattern in the function definition. That's what \emph{non-exhaustive pattern} error means. This is due to the typo I made in the third line. For Haskell compiler everything is perfectly fine: it treats the code as definition of two different function \verb+safeHead+, matching only empty lists, and \verb+safeHEad+, matching both empty and non-empty lists (though for empty list there will be a runtime exception resulting from applying \verb+head+ to an empty list). \bay{run-on; maybe delete the parenthetical}

We were unexpectedly hit by the non-exhaustive pattern problem during runtime because we used GHCi to load modules and call functions interactively. In real life, sources file containing the functions would be compiled. During the compilation process, GHC could warn us about non-exhaustive patterns in \verb+safeHead+ function and lack of type signature for accidentally defined \verb+safeHEad+ function. Warnings can be enabled using \verb+-Wall+ command line switch, as shown in Listing~\ref{l:compilation_warnings}. \bay{Note: you can turn on warnings for ghci too!} Also, to compile the code you have to add \verb+main+ function \bay{No, you can make a module, compile it, and load it into GHCi} so the whole code looks like this:
