...
\end{Verbatim}
You end up with an exception that immediately halts your program.\bay{Well, you can do some things about it} \ans{Changed} However, it is possible to create a~safe version of \verb+head+ function which will work for all possible inputs without throwing an exception. Such functions are called \emph{total functions}.
...
\end{Verbatim}
Parse errors indicate that the program violates Haskell syntax. The error message starts in the third line (not counting the blank one). It begins with name of the file and exact location of the error expressed as line and column numbers separated with colons. In this case the error is in the first line, which means our intended type annotation. The compiler complains about \verb+->+ which was not expected to appear at this location. The problem is that the name of a function should be separated from type annotation with \verb+::+. If there's no \verb+::+, the compiler assumes we are defining a~function body, treats \verb+[a]+ as a pattern binding and expects that it is followed either by \verb+=+ or \verb+|+ (a guard). \bay{Is this level of detail necessary? GHC's parse errors don't say what it was expecting.} \ans{If they did I would agree that this is too detailed. Since GHC doesn't say this I would leave this sentence as is} Of course there are lots of other ways to cause parse error, but they all are dealt with in the same way: use your favourite Haskell book or tutorial to check what syntax Haskell expects for a particular expression. Let's fix this particular mistake by adding the missing \verb+::+ symbol: \bay{I remark that sometimes it's very helpful to give parse errors to someone who is more knowledgeable about the language; they'll usually be able to figure it out, and if you're not in the habit of reading BNFs this is much more practical than "check what syntax Haskell expects": how do I do that?} \ans{Same holds for all other errors. If reader had someone who can explain to him what error messages mean he/she wouldn't be reading this paper. Anyway, I slightly modifed the sentece after the colon}
...
Failed, modules loaded: none.
\end{Verbatim}
Now that the type annotation is correct, it turns out that there's an error in third line. \emph{Not in scope} means that some variable, function or, as implied in this case, a~data constructor, is not known to the compiler. You certainly know this kind of error from different programming languages. Let's look at the definition of \verb+Maybe+ data type to understand what is wrong:\bay{How do I look up the definition of Maybe?} \ans{Fixed}
...
Just 1
\end{Verbatim}
You could implement safe versions of other unsafe functions in the same way, but there's no need to. \bay{Better separation here.} \ans{I hope it's better now. Listing demonstraiting usage of safeHead added.} There already is a library called \verb+safe+~\cite{he:safe}, which provides different safe versions of originally unsafe functions.

\bay{This could be a section break.} \ans{Honestly speaking I don't have an idea how this section could be titled.}

Let's recall the not-in-scope error that we saw earlier. It was caused by using a function which didn't exist.\bay{Or, more precisely, it existed as a type constructor, but not a data constructor.} \ans{I'm not sure if adding this information here improves the flow of discussion. Nothing changed.} Another common situation in which this error arises is when you use an existing function, but fail to import it. Let's look at a~simple piece of code in which a~different name is given to an already existing function:
...
1.0
\end{Verbatim}
In the first case, the literal \verb+1+ is interpreted as an \verb+Int+; in the second one, it is interpreted as a \verb+Double+. Both uses are correct and don't cause a type error. This works as if the \verb+fromInteger+ function defined in the \verb+Num+ class was called implicitly on the numeric literal. \bay{Not accurate in an operational sense, in the presence of optimizations, where fromInteger is never actually used.} \ans{True. I rephrased that but I'm not entirely happy with it.} This means that \verb+True && 1+ and \verb+True && fromInteger 1+ are equivalent expressions.  Let's check the type signature of \verb+fromInteger+:

\begin{Verbatim}
ghci> :t fromInteger
fromInteger :: Num a => Integer -> a
\end{Verbatim}
This means that \verb+fromInteger+ takes an \verb+Integer+ and returns a~value of \emph{any} type \verb+a+, with a~restriction that the type \verb+a+ belongs to the \verb+Num+ type class. What type exactly should be returned? That depends on the context in which \verb+fromInteger+ was applied. For example, if the return value is required to be \verb+Int+ then the implementation of \verb+fromInteger+ defined in the \verb+Num Int+ instance declaration is called. That specific implementation returns a value of type \verb+Int+. This mechanism allows integer literal to become an instance of a type belonging to \verb+Num+ type class. \bay{So, I think what you're doing here is attempting to appeal to the reader's knowledge of how type classes are resolved in order to explain how integer literals work.  You do state that readers should know basic type classes, but it might be worth mentioning briefly here.} \ans{I hope it's better now}
...
\end{listing}
which means that in order for literal \verb+1+ to be a valid parameter to \verb+(&&)+, the type returned by \verb+fromInteger+ should be \verb+Bool+. There is one problem though. The \verb+Bool+ type is not an instance of \verb+Num+ type class, as shown in Listing~\ref{l:bool}. However, it should be \bay{wording} \ans{Changed. Don't know if that's better} since the \verb+fromInteger+ function imposes a~constraint that its return value belongs to \verb+Num+ type class. This is exactly what the error message said and that is the reason why a type error occurs. \bay{Long sentence} \ans{Not anymore :) Repetition removed.} The next line of the error message suggests a~solution: \verb+Possible+ \verb+fix:+ \verb+add+ \verb+an+ \verb+instance+ \verb+declaration+ \verb+for+ \verb+(Num+ \verb+Bool)+. Indeed, if we made \verb+Bool+ type an instance of \verb+Num+ type class the problem would be gone. In some cases, this may be the solution. Deriving \verb+Bool+ to be an instance of \verb+Num+ is certainly a good exercise \bay{Well, it's not clear if its a good exercise.} \ans{Why not? I think it is for someone who hasn't done such a thing before.} that you can try out. In many other cases however this error means you wanted something different than you actually wrote. Let's assume here that we wanted \verb+1+~to denote logical truth. Fixing that is easy:

\begin{Verbatim}
ghci> True && True
True
\end{Verbatim}
\bay{Signpost here. We've solved the problem, but we're going back to talk about the error message. It might be better to jump around before you tackle the heart of the issue.  But then again, you have a nested example, so this is debatable.} \ans{Description of three remaining lines of error meaasge moved to an earlier paragraph.}
...
\end{listing}
\noindent
The error message is shown in Listing~\ref{l:rigid_types}. \bay{that sentence has no content} \ans{Sentence removed} The \verb+(b ~ a)+ notation indicates equality of the types \verb+a+ and \verb+b+. The compiler uses this notation to say \verb+Could not deduce (b ~ a)+, which means that \verb+a+ and \verb+b+ should be identical, but the type definition we provided does not guarantee it. The requirement of types \verb+a+ and \verb+b+ being of the same type is a~direct consequence of a~type signature of \verb+(==)+ which, as we recall, requires its parameters to be of the same type. The term \verb+rigid type variable+ indicates that our types \verb+a+ and \verb+b+ have been directly specified by the type annotation~\cite{he:pj} and the compiler is not free to unify them\footnote{Unification of two types means that they are assumed to be the same type.}. \bay{This might use a footnote. It's pretty advanced. What does unification mean?} \ans{Footnote added. It simplifies things a little bit but I think it's sufficient} We have already seen a correct version of this code, but we can also make it work in a~different way:
...
getFirstLetter = head
\end{Verbatim}
The \verb+head+ function has type \verb+[a] -> a+, which means it takes a list of some type \verb+a+~and returns a~single element of type \verb+a+, not a~list. However, the type signature for \verb+getFirstLetter+ requires that the return argument be a list of \verb+Char+s. Therefore, the provided signature is inconsistent with the actual return type of the function. Haskell will notice that when you try to load the code into GHCi and report an error as shown in Listing~\ref{l:mismatch_error}. It says that type annotation in the source code defines return type to be \verb+[Char]+, but the actual type inferred by the compiler is \verb+Char+. \bay{Rewrite the prior sentence.} \ans{Rewritten} There are two possible fixes for this problem. First, if we are really fine with getting a~single \verb+Char+ instead of a~\verb+String+, we can change the type signature to \verb+String -> Char+ (or \verb+[Char] -> Char+, since it's the same due to type synonyms \bay{due to type synonyms?} \ans{Yes}). On the other hand, if we really expect to get a~\verb+String+ from our function, we need to change the implementation. In this case the result of \verb+head+ should be wrapped in a~list, which can be done like this:
\begin{Verbatim}
getFirstLetter :: String -> String
getFirstLetter xs = [head xs]
\end{Verbatim}
or using a~point-free style: \bay{it's the monkey operator!} \ans{IMO telling that to the reader might be confusing and doesn't add much value}
...
\end{Verbatim}
As soon as compiler realizes that the first parameter is not a~list, it complains. It doesn't even reach the second parameter to analyze it and infer that the first list should contain \verb+Char+s. That is the reason why it uses a~list \verb+[a0]+, where \verb+a0+ represents any type. \bay{How do I tell that a type is 'generic'?} \ans{Rephreased that statement. I hope it's better.} Listing~\ref{l:wrong_param2} shows the result of swapping the two parameters. This time, the compiler is able to infer the exact type of the elements required in the second list. \bay{run-on here} \ans{Changed} It is possible because by the time compiler reaches the second parameter it has already analyzed the first one and it knows that that parameter was a~list of \verb+Char+s.
...
Another common error related to types is type ambiguity. I'll demonstrate it using the \verb+read+ function, which is used to convert \verb+String+ representation of some value to that value. The \verb+read+ function is defined in the \verb+Read+ type class and has type signature \verb+read :: Read a => String -> a+. All data types within standard Prelude, except function types and \verb+IO+ types, are instances of \verb+Read+. Every type that is an instance of \verb+Read+ type class provides its own definition of \verb+read+ function. \bay{Well, you'll get ambiguous type error even if there was only one instance of Read.} \ans{I added a footnote to clarify this} This means that the compiler must know what resulting type to expect in order to call a~correct implementation of the \verb+read+ function\footnote{Even if there was only one instance of \verb+Read+ class, the compiler wouldn't know that and you would get type ambiguity error.}. This is polymorphism, which was already discussed when we talked about \verb+fromIntegral+ function. Listing~\ref{l:ambiguity} shows that the error that occurs when the polymorphic type variable cannot be inferred to a~concrete type. In this case, the compiler doesn't know which version of \verb+read+ function should be called. \bay{long sentence} \ans{Shortened} It suggests to solve the problem by adding a~type signature. Let's try that:
...
safeHead
\end{Verbatim}
The argument \verb+[1,2,3]+ passed to the function \verb+safeHead+ couldn't be matched against any pattern in the function definition. That's what \emph{non-exhaustive pattern} error means. This is due to the typo I made in the third line. For Haskell compiler everything is perfectly fine. It treats the code as definition of two different functions: \verb+safeHead+, matching only empty lists, and \verb+safeHEad+, matching both empty and non-empty lists. Note that applying \verb+safeHEad+ to empty list will result in a runtime exception. \bay{run-on; maybe delete the parenthetical} \ans{Parenthetical shortened and changed to a separate sentence}

We were unexpectedly hit by the non-exhaustive pattern problem during runtime because GHCi has most warnings disabled by default. You can enable most of the warnings by passing \verb+-Wall+ command line switch when running \verb+ghci+ command\footnote{See \cite{he:warnings} for more details.}. Now GHCi will warn us during the compilation about non-exhaustive patterns in \verb+safeHead+ function and lack of type signature for accidentally defined \verb+safeHEad+ function. See Listing~\ref{l:compilation_warnings}. \bay{Note: you can turn on warnings for ghci too!} \ans{Why didn't I thought of that... I changed this paragraph to describe how to enable warnings in GHCI.}
